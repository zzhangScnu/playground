...
前者-碎-broken：关于testFloor单调递增；
后者-未碎-unbroken：关于testFloor单调递减。
而递推公式：res = min(res, max(dp(i-1, testFloor-1), dp(i, j-testFloor))+1)
需使得res整体最小，则该两者值应该相近，取max后的值才能尽可能小。
要使得两者值相近，从坐标轴上两个函数图像来看，即为交点处。

所以在定位楼层进行新一轮测试时，转化为二分搜索定位。
当broken > unbroken时，体现在函数图像中，是前者的曲线在后者之上，即在交点之后。此时应缩小右侧边界。
反之则应缩小左侧边界。

注意，收集结果进备忘录的地方，是在二分搜索结束后，对本次搜索的整体区间[i, j]进行记录，
而不是在过程之中对子区间[low, high]进行记录。
