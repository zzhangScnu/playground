...
- 从[0, 0]出发，走到第0行的任意一列位置，都只有一种方法。但如果该位置有障碍物，则只有0种方法，也就是走不到。
  包括障碍物之后的位置，全都被拦截走不到了；
- 同理，走到第0列的任意一行位置，也只有一种方法。但如果该位置有障碍物，则只有0种方法走到该位置及其之后的位置，也就是走不到。
遍历顺序：某个值要由左方或上方的值推导而来，即依赖于从左到右、从上而下的遍历顺序。

注意对于初始化来说，一开始实现的代码如下：
for i := 0; i < n; i++ {
	if obstacleGrid[0][i] == 0 {
		dp[0][i] = 1
}
会导致只有有障碍物的位置初始化为0，但其后的位置是1。但由题可得，障碍物及其之后的位置应该全为0。

考虑如下方式，使用【i < n && obstacleGrid[0][i] == 0】作为循环的联合结束条件。
当遇到障碍物时，跳出。后面位置的元素都不处理了，保持默认值0。
for i := 0; i < n && obstacleGrid[0][i] == 0; i++ {
	dp[0][i] = 1
}
*/
