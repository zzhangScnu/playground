...
由于只能买入一次和卖出一次，在买入时，前一天的利润一定为0，即【dp[i-1][0] - prices[i]】可以简化为【- prices[i]】，则
dp[i][1] = max(-prices[i], dp[i-1][1])
...
将dp[0][0]和dp[0][1]初始化即可。
dp[0][0]：第0天不持有，最大利润为0；
dp[0][1]：第0天持有，最大利润为 -prices[0]，而不是math.minInt。
其他保持默认值。
...
每次买入都基于初始本金0，而不是依赖于之前已经赚取的利润。也就是每次买入都是在模拟第一次买入的情况。
...
除非0再次转移为1，即有买入操作。
但买入操作排除了用状态0即卖出利润多次买入的可能性，被限制了买入都是第一次买入的场景，
故卖出也是第一次卖出。
*/
