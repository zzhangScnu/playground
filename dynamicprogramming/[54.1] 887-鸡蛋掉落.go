...
情况2：有多个鸡蛋。此时可以遍历所有楼层，基于每个楼层进行测试和择优。
	  直到剩下一个鸡蛋，此时最小操作次数即为剩余待测试楼层层数。
 	  解空间可以表示为二叉树。
...
第一层扔				(1, 0)	  (2, 5)
							碎/	   \未碎
第二层扔					(1, 1)	   (2, 4)
	鸡蛋数--，向下测试，仅剩一层楼       鸡蛋数不变，向上测试，仍剩四层楼
...
注意，每个dp[i][j]都是独立测试j层楼范围的最优解，与这些楼层在建筑中的实际物理位置无关。
循环中的j范围[1, j]表示在当前j层范围内的相对位置。所以每个子问题都是从1到j的遍历范围。
