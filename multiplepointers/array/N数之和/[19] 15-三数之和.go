...
/**
思路：
1. 创造数组非递减有序条件——有序数组，就应该第一时间想到【双指针】；
2. 固定第一个值，剩余的两个值用2Sum思路解决；
3. 跟两数之和不同的地方在于，结果集不能有重复元素。所以用哈希，去重逻辑会比较冗余。
...
注意点2：非重复组合的控制
1. 对于剩余的两个值，可以在2Sum方法里控制。需要注意的是要在收集结果后再跳过重复值，否则会遗漏结果；
...
for low < high && nums[low] == lowVal
1. 用局部变量，固定基准值；
2. 从当前值本身开始对比，这样即使后面没有重复元素，也能使得当前指针往前进一步，从下个位置重新开始（兼容当前位置在未重复情况下也能推进）；
3. 控制边界，避免越界；
4. 对于nums[i] == num，本质上是nums[i]和nums[i-1]的对比。而不是nums[i]和nums[i+1]。前者是避免出现重复结果元组，后者是跳过(0,0,0)这样有重复元素的结果。
*/
