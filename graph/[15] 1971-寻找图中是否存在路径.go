...
func validPath(n int, edges [][]int, source int, destination int) bool {
	var buildGraph func(n int, edges [][]int) [][]int
	var travel func(graph [][]int, source, destination int) bool
	buildGraph = func(n int, edges [][]int) [][]int {
		graph := make([][]int, n)
	...
	visited := make([]bool, n)
	travel = func(graph [][]int, source, destination int) bool {
		if visited[source] {
			return false
		}
		visited[source] = true
		...
		for _, next := range graph[source] {
			if travel(graph, next, destination) {
				return true
			}
		}
		return false
	...
	return travel(graph, source, destination)
}
