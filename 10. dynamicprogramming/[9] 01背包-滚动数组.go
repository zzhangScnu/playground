...
将i消去后，
- j：剩余容量；
- dp[j]：剩余容量为j的背包中物品的最大总价值；

递推公式：
dp[j] = max(dp[j], dp[j-weight[i]] + value[i])

初始化：
dp[0] = 0，表示剩余容量为0时，无法装下任何物品。其余单元格的值都可以通过计算推导而来，无需显式初始化，且也不能初始化为其他值，可能会造成计算错误。
推导方向：
物品 -> 从前到后；
背包 -> 从后到前：
因为滚动数组的本质是屏蔽了对物品选取的表达，仅看当前剩余容量为j时的背包情况。
...
可举例推演一下做简单证明。
因为是从最大值开始逆向遍历背包的剩余容量，且终止条件是当前物品的重量，所以不需要像二维DP数组一样判断当前物品是否能放入当前背包。

遍历方向：
背包遍历方向：从后到前
物品&背包两个维度的for循环：是否可以颠倒？
若先物品再背包：
对第i个物品，算一遍各种剩余容量下的背包最大总价值。再对第i+1个物品，基于取或不取i，算一遍各种剩余容量下的背包最大总价值。以此类推……
每轮dp[0..W]的计算，是基于上一轮的dp[0..W]的。
满足了DP数组的定义：考虑物品i时，dp数组是物品i-1取/不取时，剩余容量分别为[0...W]时的最大总价值。
若先背包再物品：
由上述推导方向可知，遍历方向应为 W -> 0，才能避免重复放入同一个物品。
对每个背包剩余容量，计算dp[W...0]。
当剩余容量为j时，考虑每个物品的放入情况。但由于dp[j]依赖的dp[0...j-1]此时还没基于每个物品的取用情况计算出来，无法依赖重叠子问题的计算结果来推导值。
