...
/**
思路：
标准Dijkstra寻找最短路径题目，注意：

1. 需寻找使所有节点都收到信号的时间 -> 从起点到各节点的最短路径中的最大值；
2. 节点范围为[1, n]，所以邻接表graph和最短路径distance的大小初始化为n + 1；
3. 最终遍历distance寻找结果时，从1开始，因为0无业务语义：
   若有任一一个元素仍为初始值math.MaxInt，则表示存在节点与起点不联通，故无法使所有节点收到信号，返回-1；
   否则返回其中的最大值。
*/
