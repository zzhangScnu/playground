...
//1、这题给的是⽆向图，也可以⽤ Dijkstra 算法吗？
//2、更重要的是，Dijkstra 算法计算的是最短路径，计算的是最⼩值，这题让你计算最⼤概率是⼀个最⼤值，
//怎么可能⽤ Dijkstra 算法呢？
//问得好！
//⾸先关于有向图和⽆向图，前⽂ 图算法基础 说过，⽆向图本质上可以认为是「双向图」，从⽽转化成有向
//图。
//重点说说最⼤值和最⼩值这个问题，其实 Dijkstra 和很多最优化算法⼀样，计算的是「最优值」，这个最优
//值可能是最⼤值，也可能是最⼩值。
//标准 Dijkstra 算法是计算最短路径的，但你有想过为什么 Dijkstra 算法不允许存在负权重边么？
//因为 Dijkstra 计算最短路径的正确性依赖⼀个前提：路径中每增加⼀条边，路径的总权重就会增加。
//这个前提的数学证明⼤家有兴趣可以⾃⼰搜索⼀下，我这⾥只说结论，其实你把这个结论反过来也是 OK
//的：
//如果你想计算最⻓路径，路径中每增加⼀条边，路径的总权重就会减少，要是能够满⾜这个条件，也可以⽤
//Dijkstra 算法。
//你看这道题是不是符合这个条件？边和边之间是乘法关系，每条边的概率都是⼩于 1 的，所以肯定会越乘越
//⼩。
//只不过，这道题的解法要把优先级队列的排序顺序反过来，⼀些 if ⼤⼩判断也要反过来，我们直接看解法代
//码吧
