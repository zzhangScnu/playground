...
注意X1 / Y1 / X2 / Y2需初始化为对应的值，以便做min / max。

2. 获取小矩形组成的图形的顶点 ->
	- 如果是大矩形的顶点：只会遍历一次；
	- 如果是大矩形的内部节点：因为会被多个矩形共享，所以会被遍历偶数次。如2次(位于大矩形的边缘)、4次(位于大矩形的内部)等。
   所以维护一个HashMap，在遍历小矩形顶点的过程中收集顶点：
	- 如果没有该顶点，则加入；
	- 如果已有该顶点，说明顶点被多个小矩形共享，不是大矩形的顶点，则删除。
   最后看看HashMap中的顶点数量：
	- 如果不等于4个，说明小矩形们组成的并不是一个标准的大矩形；
	- 如果等于4个，再看HashMap中的顶点与大矩形的顶点是否一一对应。
*/
