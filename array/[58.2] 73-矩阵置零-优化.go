...
row0 和 col0 的核心作用：解决第一行 / 第一列的 “自我标记” 歧义，记录原本在第一行 / 第一列中是否有0，而不是后续被赋值的0。这样才能决策是否需要将其所在的行 / 列全置为0。
非第一行（i≥1）的置零依据：matrix[i][0]（第 i 行的第一列元素）
非第一列（j≥1）的置零依据：matrix[0][j]（第 j 列的第一行元素）
