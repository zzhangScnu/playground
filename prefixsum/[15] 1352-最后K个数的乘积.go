...
// ProductOfNumbers 设计一个算法，该算法接受一个整数流并检索该流中最后 k 个整数的乘积。
//
...
思路：
前缀积数组。
跟前缀和唯一的不同就是，遇到乘数为0的情况，所有前缀积都需要清零。
如果按正常思路实现，即对每个元素 * 0，时间复杂度会达到O(n)。
有个小技巧，就是将数组清空，回归初始化状态。
在获取最后K个数的乘积时，因为题目限定k合法，即当前列表中始终至少包含k个数字。
因为初始化时令preProduct[0] = 1，方便前缀积计算的边界处理，数组长度会比实际添加的元素长度大1。
那么如果len(this.preProduct)-1 < k，说明乘数中有0，直接返回0即可。
*/
