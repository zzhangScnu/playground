...
/**
思路：
因为 nums1 和 nums2 是升序排序的，所以 [nums1[0], nums2[0]] 一定是和最小的一对数字，
将其以【和，nums1当前索引i，nums2当前索引j】加入最小堆，并且标记为已访问，避免重复。

本质难点：取全局最小组合后，不知道应当扩展 i 还是 j 的坐标继续找最小。

维护一个最小堆，以数字对的和进行升序排序。
对最小堆开启循环操作：
取出和最小的一对数字，加入结果集并消耗 k 的次数。
同时对 nums1 的 i 和 nums2 的 j 进行扩展：二维平面中，向右或向下走
注意越界和重复访问控制。

PS：总是会忘记 Golang 里面堆操作的正确写法，不能直接操作 minHeap 的方法！
*/
